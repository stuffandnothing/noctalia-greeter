...
void tryHomeStateLog() {
  ...
  std::string path =
      std::string(home) + "/.local/state/noctalia-greeter/greeter.log";
  tryOpenLogFile(path.c_str());
}
...
void emergencyLogBootstrap(int argc, char *argv[]) {
  ...
  msg << "=== noctalia-greeter bootstrap pid=" << ::getpid()
  ...
  msg << " GREETD_SOCK="
      << (std::getenv("GREETD_SOCK") ? std::getenv("GREETD_SOCK") : "unset");
  msg << " XDG_VTNR="
      << (std::getenv("XDG_VTNR") ? std::getenv("XDG_VTNR") : "unset");
  msg << " ===\n";
...
void initLogging() {
  ...
  if (!g_logPaths.empty()) {
    ...
    std::fprintf(stderr, "[warn] [log] no log file opened; check permissions "
...
static void writeLogLine(std::string_view tag, std::string_view level,
